weekBegin = 1;                      // First day of week. (0 = Sunday, 1 = Monday etc)
...
dividerLine = "#dadada";            // Colour of lines dividing Day Labels and Weekends
...
monthBackground = "#5C5082";  //"#DCD9C8"; // "#8C1229";        // Background colour of month strap
monthColor = "#FFFFFF"; //"#404040"; //"#FFFFFF";             // Text colour of Month title
monthFace = "arial, sans-serif";    // Font of Month title
monthSize = "8pt";                 // Font size of Month title
...
dateBackground = "#FFFFFF"; //"#f3eecb";         // Background colour of full date cells
dateBackgroundH = "#fddcCC";        // Background colour of full date cells on Mouseover
...
weekendBackground = "#FFFFFF"; //"#f3eecb";      // Background colour of full date cells
weekendFace = "arial,sans-serif";   // Font of weekend dates
weekendSize = "8pt";                // Font size of weekend  dates
weekendColor = "#adadad";           // Text colour of weekend  dates

offBackground = "#F5F3EE"; //"#FCFAEB";          // Background colour of full date cells
...
todayBackground = "#FCC0AB";        // Background colour of today's date. todayHighlight must be set to 1.
...
todayColor = "#8C1229";             // Text colour of today's date. todayHighlight must be set to 1.
...
var rcMonths=["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
var rcShortMonths=["Ene", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Ago", "Sep", "Oct", "Nov", "Dic"];
var rcDays=["D","L","M","X","J","V","S"]
...
rcStyles+='table.calendario{border:0}';
rcStyles+='table.calendario tbody tr td{line-height:13px;border:0;}';
rcStyles+='.monthhead{border-color:'+monthBackground+'; background-color:'+monthBackground+';font-family:'+monthFace+';color:'+monthColor+';font-size:'+monthSize+';font-weight:'+dayWeight+';text-decoration:none;}';
...
item++;
  ...
  setCal='<table cellpadding="0" cellspacing="0"><tr><td>';
  ...
  if(popStatus>0){
    setCal+='&nbsp;<a href="javascript:void(0)" onClick="showCalendar(\'\','+item+')"><img src="img/icons/cal.gif" border="0"  alt="Calendario" /><\/a>';
  }
  setCal+='</p><\/td><\/tr><\/table>';
  setCal+='<\/div>';  
  visible=(popStatus>0)?"hidden":"visible";
//  setCal+='<div z-index="1000" id="calendarBox'+item+'" name="calendarBox'+item+'" style="visibility:'+visible+'; position:absolute;left:0px;top:0px;z-index:999"><\/div>';
  setCal+='<div z-index="1000" id="calendarBox'+item+'" name="calendarBox'+item+'" style="visibility:'+visible+'; position:relative;z-index:999"><\/div>';
  setCal+='</td></tr></table>';
...
function showCalendar(when,item){
  ...
  if(when==""||when==null){
    ...
    var calendarDateObj = eval(obj+'("calendarDate'+item+'")')
	getOffset(calendarDateObj,'top');
    getOffset(calendarDateObj,'left');
    getOffset(calendarDateObj,'width');
...
function getRCCalendar(year,month,item){
  ...
  if(year==null||year==""){
  	...
  	year=yearCorrect(now.getYear());month=now.getMonth()}
  ...
  doCalc='<table class="calendario" cellpadding="0" cellspacing="0"><tr style="border:0px"><td colspan="2">';
  doCalc+='<table class="calendario" cellpadding="3" cellspacing="0"';if(outerBorder!="0"){doCalc+='style="border:1px solid '+outerBorder+'"';}doCalc+='>';
  doCalc+='<tr style="border:0px">';
  doCalc+='<td colspan="7" class="monthhead"><table border="0" cellpadding="0" cellspacing="0" width="100%"><tr style="border:0px">';
  if(scrollingMonth==1){
  	doCalc+='<td class="monthhead"><a class="monthhead" href="javascript:void(0)" onClick="getRCCalendar('+year+','+(month-1)+','+item+')">'+minusSymbol+'<\/a><\/td>';}
	 
  doCalc+='<td align="center" class="monthhead">'+rcMonths[showMonth.getMonth()]+" "+year+'<\/td>';
  
  if(scrollingMonth==1){doCalc+='<td class="monthhead" align="right"><a class="monthhead" href="javascript:void(0)" onClick="getRCCalendar('+year+','+(month+1)+','+item+')">'+plusSymbol+'<\/a><\/td>';} 
  ...
  doCalc+='<\/table><\/td><\/tr><tr style="border:0px">';
  if(popStatus==1){doCalc+='<td width="50%"><a class="close" href="javascript:void(0)" onClick="closeCalendar('+item+')">Cerrar</a><\/td>';}
  if(dateBox==1){doCalc+='<td width="50%" align="right"><a class="close" href="javascript:void(0)" onClick="clearForm('+item+')">Limpiar Forma</a><\/td>';}
  doCalc+='<\/table>';
...
/*Es la misma funci\F3n que doRCCalendar, con la salvedad de que recibe como par\E1metro una cadena
la cual representa la fecha que debe contener*/

function doRCCalendar2(cadena){
item++;
  setCal='<table><tr style="padding:0;border:0px!important"><td style="padding:0;border:0px!important;" >';
  setCal+='<div z-index="999" id="calendarDateDiv'+item+'">';
  setCal+='<table><tr style="padding:0;border:0px!important;><td style="padding:0;border:0px!important;" ><p>';
  if(dateBox==1){setCal+='<input type="text" id="calendarDate'+item+'" name="calendarDate'+item+'" value="'+cadena+'">';}else{setCal+='&nbsp;';}   
  if(popStatus>0){
    setCal+='&nbsp;<a href="javascript:void(0)" onClick="showCalendar(\'\','+item+')"><img src="img/icons/cal.gif" border="0"  alt="Calendario" /><\/a>';
  }
  setCal+='</p><\/td><\/tr><\/table>';
  setCal+='<\/div>';  
  visible=(popStatus>0)?"hidden":"visible";
  setCal+='<div z-index="1000" id="calendarBox'+item+'" name="calendarBox'+item+'" style="visibility:'+visible+'; position:absolute;z-index:999"><\/div>';
  setCal+='</td></tr></table>';
